...
describe("TextExpander", () => {
  ...
  it("renders short text without truncation", () => {
    ...
    expect(
      screen.getByText((content) => content.includes(shortText))
    ).toBeInTheDocument();
  ...
  it("truncates long text correctly", () => {
    ...
    expect(
      screen.getByText((content) =>
        content.includes(truncatedText.substring(0, 50))
      )
    ).toBeInTheDocument();
    expect(screen.getByText("Show more")).toBeInTheDocument();
  });
  ...
  it('expands text when "Show more" is clicked', async () => {
    ...
    expect(
      screen.getByText((content) => content.includes(firstSentence))
    ).toBeInTheDocument();
  });
  ...
  it('collapses text when "Show less" is clicked', async () => {
    ...
    expect(
      screen.getByText((content) => content.includes(truncatedBeginning))
    ).toBeInTheDocument();
  });
